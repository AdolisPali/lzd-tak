...
sudo NEEDRESTART_MODE=a DEBIAN_FRONTEND=noninteractive apt install curl python3 python3-pip python3-venv rabbitmq-server openssl nginx ffmpeg libnginx-mod-stream python3-dev postgresql-postgis pgloader -y
...
INSTALL_ZEROTIER=0
echo "${GREEN}Skipping ZeroTier install (non-interactive run)${NC}"
...
INSTALL_MUMBLE=0
echo "${GREEN}Skipping Mumble Server install (non-interactive run)${NC}"
